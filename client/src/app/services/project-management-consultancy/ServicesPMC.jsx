...
export default function ServicesPMC() {

      const [pmcServices, setpmcServices] = useState([]);
        
          useEffect(() => {
            fetch("/data/services/pmc/Service.json")
              .then((res) => res.json())
              .then((data) => setpmcServices(data));
          }, []);
  ...
  return (
     <section className="common-section flex flex-col justify-center bg-[#F9FAFB]">
      <div className="text-center mb-10">
        ...
        <h2 className="center-section-heading">COMPREHENSIVE PMC SOLUTIONS FOR <br />EVERY PROJECT STAGE </h2>

      </div>

      {/* Services Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 gap-y-12 justify-evenly items-stretch">
        {pmcServices.map((item) => (
          <div
            key={item.id}
          ...
          >
            ...
            <div className="flex flex-col grow px-4 py-4 text-left">
              ...
              <ul className="">
                {item.points?.map((p, j) => (
                  <li key={j} className="flex items-start gap-2 bulletPoints text-[#3d3d3d]">
